...
class Relation(Base):
    ...
    source_id = Column(
        Integer,
        ForeignKey("functional_items.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    target_id = Column(
    ...
    # Relationships
    source = relationship(
        "FunctionalItem", foreign_keys=[source_id], backref="outgoing_relations"
    )
    target = relationship(
        "FunctionalItem", foreign_keys=[target_id], backref="incoming_relations"
    )

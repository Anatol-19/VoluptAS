...
class SettingsProfile:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            'google_json_path': str(self.google_json_path) if self.google_json_path else None,
    ...
    def from_dict(cls, data: Dict) -> 'SettingsProfile':
        return cls(
            ...
            name=data['name'],
            description=data['description'],
            zoho_env_path=Path(data['zoho_env_path']) if data.get('zoho_env_path') else None,
            google_json_path=Path(data['google_json_path']) if data.get('google_json_path') else None,
            qase_env_path=Path(data['qase_env_path']) if data.get('qase_env_path') else None,
...
class ProjectManager:
    ...
    def load(self):
        ...
        if self.current_project_file.exists():
            self.current_project_id = self.current_project_file.read_text(encoding='utf-8').strip()
    ...
    def save(self):
        ...
        if self.current_project_id:
            self.current_project_file.write_text(self.current_project_id, encoding='utf-8')
    
    def create_project(self, project_id: str, name: str, description: str, 
                       settings_profile: str = 'production') -> ProjectConfig:

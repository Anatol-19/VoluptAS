...
class MergeStrategy:
    ...
    @classmethod
    def merge_functional_item(
        cls, local_item, zoho_data: Dict, merge_policy: str = "smart"
    ) -> Dict[str, Any]:
        """
    ...
    def _merge_list_field(
        cls, local_value: Optional[str], zoho_value: Optional[str]
    ) -> Optional[str]:
        """
        ...
        # Парсим оба списка
        local_items = set(
            item.strip() for item in local_value.split(",") if item.strip()
        )

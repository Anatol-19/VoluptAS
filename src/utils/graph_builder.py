...
def build_graph_from_attributes(
    items: List[FunctionalItem], relations: Optional[List] = None
    ...
    logger.info(
        f"Building graph from {len(items)} items, {len(relations) if relations else 0} relations"
    )
    ...
    if relations:
        for rel in relations:
            if (
                rel.active
                and rel.source_id in items_by_id
                and rel.target_id in items_by_id
            ):
                edges.append(
                    {
                        "from": rel.source_id,
...
def find_parent_by_title(
    items: List[FunctionalItem], title: str, type_filter: str
) -> Optional[FunctionalItem]:
    """
    ...
    for item in items:
        ...
        if item.functional_id.upper().startswith(
            title_clean.upper().replace(" ", "-").replace("_", "-")
        ):
            return item
...
def build_hierarchy_graph(
    items: List[FunctionalItem], root_type: str = "Module"
) -> Tuple[List[Dict], List[Dict]]:
    """
...
def get_item_neighbors(
    item: FunctionalItem, items: List[FunctionalItem]
) -> Tuple[List[FunctionalItem], List[FunctionalItem]]:
    """
    ...
    for i in items:
        if i.parent_id == item.id:
            ...
            i.module
            and i.module.lower() == item.title.lower()
            ...
            i.feature
            and i.feature.lower() == item.title.lower()
            and item.type == "Feature"
        ):
            if i not in children:
                children.append(i)

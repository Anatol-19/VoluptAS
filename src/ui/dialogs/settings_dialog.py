...
class SettingsDialog(QDialog):
    ...
    def __init__(self, project_manager=None, parent=None):
        ...
        if project_manager:
            ...
            if current_project:
                ...
                if profile:
                    self.zoho_env_path = (
                        profile.zoho_env_path or Config.get_credentials_path("zoho.env")
                    )
                    self.google_json_path = (
                        profile.google_json_path
                        or Config.get_credentials_path("google_credentials.json")
                    )
                    self.qase_env_path = (
                        profile.qase_env_path or Config.get_credentials_path("qase.env")
                    )
                else:
    ...
    def create_zoho_tab(self):
        ...
        show_secret_btn.clicked.connect(
            lambda: self.toggle_password(self.zoho_client_secret_edit)
        )
        ...
        show_refresh_btn.clicked.connect(
            lambda: self.toggle_password(self.zoho_refresh_token_edit)
        )
        ...
        self.zoho_auth_code_edit.setPlaceholderText(
            "Введите Authorization Code из Zoho OAuth"
        )
    ...
    def create_qase_tab(self):
        ...
        show_token_btn.clicked.connect(
            lambda: self.toggle_password(self.qase_token_edit)
        )
    ...
    def load_zoho_settings(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Ошибка", f"Не удалось загрузить настройки Zoho:\n{e}"
            )
    ...
    def load_google_settings(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Ошибка", f"Не удалось загрузить Google JSON:\n{e}"
            )
    ...
    def load_qase_settings(self):
        ...
        try:
            ...
            self.qase_url_edit.setText(
                creds.get("QASE_BASE_URL", "https://api.qase.io/v1")
            )

        except Exception as e:
            QMessageBox.warning(
                self, "Ошибка", f"Не удалось загрузить настройки Qase:\n{e}"
            )
    ...
    def refresh_access_token(self):
        ...
        if not all([client_id, client_secret, refresh_token]):
            QMessageBox.warning(
                self, "Ошибка", "Заполните Client ID, Client Secret и Refresh Token!"
            )
            return
    ...
    def fetch_projects(self):
        ...
        if not access_token or not portal_name:
            QMessageBox.warning(
                self, "Ошибка", "Сначала обновите Access Token и укажите Portal Name!"
            )
            return
    ...
    def on_projects_received(self, projects, error):
        ...
        if error:
            QMessageBox.critical(
                self, "Ошибка", f"Не удалось загрузить проекты:\n{error}"
            )
            self.status_label.setText(f"❌ Ошибка: {error}")
            self.status_label.setStyleSheet("color: red; padding: 5px;")
            return
...
"""
        try:
            ...
            QMessageBox.critical(
                self,
                "Ошибка",
                f"Не удалось сохранить Zoho credentials (zoho.env):\n{e}",
            )
        ...
        if content:
            try:
                ...
                QMessageBox.critical(
                    self,
                    "Ошибка",
                    f"Не удалось сохранить Google credentials (google_credentials.json):\n{e}",
                )
...
"""
        try:
            ...
            QMessageBox.critical(
                self,
                "Ошибка",
                f"Не удалось сохранить Qase credentials (qase.env):\n{e}",
            )

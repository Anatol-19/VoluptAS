        ...
        for i in range(self.projects_list.count()):
            ...
            if project:
                match = (
                    ...
                    or search_text in (project.description or "").lower()
                    or search_text in project_id.lower()
                )
...
class NewProjectDialog(QDialog):
    ...
    def init_ui(self):
        ...
        for profile in profiles:
            ...
            self.profile_combo.addItem(
                f"{emoji} {profile.name} - {profile.description}", profile.id
            )
    ...
    def create_project(self):
        ...
        if not re.match(r"^[a-z0-9_-]+$", project_id):
            QMessageBox.warning(
                self, "Ошибка", "ID проекта может содержать только: a-z, 0-9, _, -"
            )
            return
        ...
        if project_id in self.project_manager.projects:
            QMessageBox.warning(
                self, "Ошибка", f'Проект с ID "{project_id}" уже существует'
            )
            return

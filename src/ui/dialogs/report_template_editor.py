...
class TemplateEditorDialog(QDialog):
    ...
    def __init__(self, session, template=None, parent=None):
        ...
        self.setWindowTitle(
            "Редактор шаблона отчёта"
            if self.is_new
            else f"Редактирование: {template.name}"
        )
    ...
    def init_ui(self):
        ...
        self.content_edit.setPlaceholderText(
            "# Введите markdown текст шаблона...\n\n## Раздел 1\n..."
        )
...
class TemplateManagerDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["Название", "Тип", "Описание", "Активен", "Обновлён"]
        )
    ...
    def load_templates(self):
        ...
        for row, template in enumerate(templates):
            ...
            self.table.setItem(
                row, 3, QTableWidgetItem("✓" if template.is_active else "")
            )
    ...
    def edit_template(self):
        ...
        template = (
            self.session.query(ReportTemplate).filter_by(name=template_name).first()
        )
            ...
            try:
                ...
                if template:
                    ...
                    QMessageBox.information(
                        self, "Успех", f'✅ Шаблон "{template_name}" удалён'
                    )
            except Exception as e:
                self.session.rollback()

...
class GraphViewWindow(QMainWindow):
    ...
    def load_graph(self):
        ...
        self.statusBar().showMessage(
            f"Загружено: {len(items)} узлов, {len(relations)} связей"
        )
    ...
    def refresh_graph(self):
        ...
        ax.axis("off")
        ax.set_title(
            f"Граф связей ({len(filtered_graph.nodes())} узлов, {len(filtered_graph.edges())} связей)",
            fontsize=14,
            fontweight="normal",
            color="#d1d5db",
        )
        ...
        self.canvas.draw()
        self.statusBar().showMessage(
            f"Отображено: {len(filtered_graph.nodes())} узлов, {len(filtered_graph.edges())} связей"
        )
